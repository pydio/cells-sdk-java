...
public class RestListProcessesResponse {
    ...
    private List<RestProcess> processes = new ArrayList<>();

    public RestListProcessesResponse() {
    }

    public RestListProcessesResponse processes(@javax.annotation.Nullable List<RestProcess> processes) {
    ...
    public void setProcesses(@javax.annotation.Nullable List<RestProcess> processes) {
        this.processes = processes;
    }
     ...
     * @throws IOException if the JSON Element is invalid with respect to RestListProcessesResponse
     */
    public static void validateJsonElement(JsonElement jsonElement) throws IOException {
        if (jsonElement == null) {
            if (!RestListProcessesResponse.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
        ...
        for (Map.Entry<String, JsonElement> entry : entries) {
            if (!RestListProcessesResponse.openapiFields.contains(entry.getKey())) {
                throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `RestListProcessesResponse` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
            }
        }
        ...
        if (jsonObj.get("Processes") != null && !jsonObj.get("Processes").isJsonNull()) {
            ...
            if (jsonArrayprocesses != null) {
                ...
                for (int i = 0; i < jsonArrayprocesses.size(); i++) {
                    RestProcess.validateJsonElement(jsonArrayprocesses.get(i));
                }
                ;
            }
        }
